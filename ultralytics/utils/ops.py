...
def scale_boxes_old(img1_shape, boxes, img0_shape, ratio_pad=None, padding=True):
    """
...
def non_max_suppression(
    ...
    box_label_last = False
    if prediction.shape[1] > prediction.shape[2] and prediction.shape[2] == 85:
        # is yolov5, yolov6, yolov7, shape(1, 250000, 85), 
        return non_max_suppression_yolov5(prediction, conf_thres=conf_thres,
                                          iou_thres=iou_thres, classes=classes,
                                          agnostic=agnostic, multi_label=multi_label,
                                          labels=labels, max_det=max_det, nm=0)
    elif prediction.shape[2] == 84:
        # is ppyoloe
        box_label_last = True
    else:
        # is yolov8
